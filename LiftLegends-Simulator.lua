...
local autoTab = main:AddTab('Auto-Tools')
local farmTab = main:AddTab('Farming')
local teleportTab = main:AddTab('Teleports')
local miscTab = main:AddTab('Misc')

autoTab:AddSwitch('Auto Click',function(bool)
    ClickAuto = bool;
end)
...
autoTab:AddSwitch('Auto Sell',function(bool)
    SellAuto = bool;
end)
...
farmTab:AddSwitch('Farm Stanima',function(bool)
    runAuto = bool;
end)

farmTab:AddSwitch('Farm Endurance',function(bool)
    AutoEndurance = bool;
end)

autoTab:AddSwitch('Auto Kill',function(bool)
    killAutomatic = bool
end)
...
miscTab:AddSwitch('Fast Click',function(bool)
    FastClickOpt = bool
    gamepass.Value = bool
end)
...
local AreaLocs = teleportTab:AddDropdown("Areas", function(object)
    for i,v in ipairs(Areas) do 
		if object == v.Name then 
			SelectedArea = tonumber(i);
			localPlayer.Character:MoveTo(v.PartPos)
		end
	end
end)
...
local tierLocs = teleportTab:AddDropdown("Egg Tiers", function(object)
    for i,v in ipairs(TiersEgg) do 
		if object == v.Name then 
			SelectedTier = tonumber(i);
			localPlayer.Character:MoveTo(v.PartPos)
		end
	end
end)
...
spawn(function()
	for i,v in ipairs(TiersEgg) do 
		tierLocs:Add(tostring(v.Name))
	end
end)

teleportTab:AddButton("Teleport to Area",function()
	if Areas[SelectedArea] and Areas[SelectedArea].Name then 
		localPlayer.Character:MoveTo(Areas[SelectedArea].PartPos)
	end
end)

teleportTab:AddButton("Teleport to Egg/Tier",function()
	if TiersEgg[SelectedArea] and TiersEgg[SelectedArea].Name then 
...
autoTab:AddSwitch('Auto-Acsension',function(bool)
    autoAcsensionTOG = bool;
end)

spawn(function()
    while true do 
        if autoAcsensionTOG then 
...
local folderAutobuy = autoTab:AddFolder("Auto Buy | Tools")

folderAutobuy:AddSwitch('AutoBuy-Strength',function(bool)
    StrengthAutobuy = bool;
end)

folderAutobuy:AddSwitch('AutoBuy-Endurance',function(bool)
